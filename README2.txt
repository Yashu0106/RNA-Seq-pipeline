...
# Difference between GTF and GFF and GFF3
...
# HTseq only works with GTF/GFF2/GFF, NOT GFF3.



# Troubleshooting for some errors I encountered: 
# During STAR, if used SortByCoordinate.bam, may cause HTSeq warning because the sorting missing adjacent aligned mates by position.
# Use '--order pos' to ask HTSeq read align mates by position, HTSeq will be slow and use larger MEM if select read by position)
# This may happen for NCBI-sourced file: for GFF file, "gene_id" may be written as "gene". use "-i=gene" in the command lines and try again.
# For GTF file, error such as "start larger than end" may pop up, indicating the forth column "start position" number is larger than the fifth column "end position" number. Go to the line that error is indicating, and reverse the numbers of the two positions to fix the error.
# For NCBI sourced gtf file, GCF_000003745.3_12X_genomic_edited.gtf is the one after fixing the "start larger than end" error.
# In HTSeq, Use '--stranded=no' for illimna mRNA-Seq. If you are not sure, see the library preparation protocol of mRNA-Seq that your platform used, to see if they are strand-specific nor non-strand specific.
# HTSeq has defaultly made suitable for Ensembl GTF file, so using Ensembl sourced file may be easier than using NCBI sourced file for HTSeq counts.
